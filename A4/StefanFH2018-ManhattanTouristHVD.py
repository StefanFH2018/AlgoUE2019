...
bin_ich_in_diagonal_matrix = False
...
for line_input in file_input:
    ...
    if "G_down" in line_input:
        ...
        continue
    elif "G_diag" in line_input:
        bin_ich_in_diagonal_matrix = True
        continue
    elif "---" in line_input:
        bin_ich_in_down_matrix = bin_ich_in_right_matrix = bin_ich_in_diagonal_matrix = False
        continue
    ...
    elif bin_ich_in_diagonal_matrix == True:
        diagonal_matrix.append(line_input.split())

down_matrix = float_list(down_matrix)
right_matrix = float_list(right_matrix)
diagonal_matrix = float_list(diagonal_matrix)

length_column = len(down_matrix[1])
length_line = len(right_matrix)
length_diag = len(diagonal_matrix)
...
for line in range(1, length_line, 1):
    ...
    for column in range(1, length_column, 1):
        ...
        diagonal = calculation_list[line -1][column -1] + diagonal_matrix[line - 1][column - 1]
        ...
        if horizontal >= vertical and horizontal >=diagonal:
            ...
            calculation_list[line].append(diagonal)
